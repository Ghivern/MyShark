...
ProTree* DissectorFrame::Dissect(raw_t *raw, dissResList_t *dissResList,qint64 index,Info *info){
    ...
    if(info == NULL){
        ...
        dissResEth->AddToProtocolStackWithSE("eth",0);
        dissResList->append(dissResEth);
    }else{
        proTree = new ProTree();
        DissResEth *dissResEth = (DissResEth*)(dissResList->at(index));
        proTree->AddItem("eth",DissectorFrame::MsgTop(raw,index,info),0,dissResEth->GetHeadersLen() - 1); // -
        ...
        proTree->AddItem("eth",DissectorFrame::MsgStrTime(raw)); // - -
        proTree->AddItem("eth",DissectorFrame::MsgEpochTime(raw)); // - -
        ...
        proTree->AddItem("eth",DissectorFrame::MsgFrameLen(raw));  // - -
        proTree->AddItem("eth",DissectorFrame::MsgCapLen(raw));  // - -
...
//Msg方法
QString DissectorFrame::MsgTop(raw *raw, qint64 index ,Info *info){
    uchar bit = DissectorFrame::FlagGetShowBits();
    return QString::asprintf("Frame %lld: %d bytes on wire",index,raw->pkthdr->len)
            + (bit == 0 ? " , ":QString::asprintf(" (%d bits), ",raw->pkthdr->len * 8))
            + QString::asprintf("%d bytes captured",raw->pkthdr->caplen)
            + (bit == 0 ? " ":QString::asprintf(" (%d bits) ",raw->pkthdr->caplen * 8))
...
QString DissectorFrame::MsgStrTime(raw_t *raw){
    time_t time_sec = raw->pkthdr->ts.tv_sec;
    struct tm *local;
    local = localtime(&time_sec);
    char strTime[128];
    strftime(strTime,64, "%Y-%m-%d %H:%M:%S", local);
    return QString::asprintf("Arrival time : %s.%ld",strTime,raw->pkthdr->ts.tv_usec);
}

QString DissectorFrame::MsgEpochTime(raw_t *raw){
    return QString::asprintf("Epoch time : %ld.%ld seconds"
                             ,raw->pkthdr->ts.tv_sec,raw->pkthdr->ts.tv_usec);
}
...
QString DissectorFrame::MsgFrameLen(raw_t *raw){
    return QString::asprintf("Frame length : %d bytes (%d bits)",raw->pkthdr->len,raw->pkthdr->len*8);
}

QString DissectorFrame::MsgCapLen(raw_t *raw){
    return QString::asprintf("Capure length : %d bytes (%d bits)",raw->pkthdr->caplen,raw->pkthdr->caplen*8);
}
