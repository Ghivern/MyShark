...
ProTree* DissectorEth::Dissect(DissResList *dissResList,qint64 index, Info *info){
    ...
    eth_hdr *header = GetHdr(dissResList->at(index));
    ...
    case (ushort)0x0800:
        DissectorIp::Dissect(dissResList->at(index),proTree,info);
        break;
    case (ushort)0x0806:
...
//Get方法
eth_hdr* DissectorEth::GetHdr(DissRes *dissRes){
...
quint32 DissectorEth::GetIntFCS(DissRes *packet){
    return ntohl(*(packet->GetData() + packet->GetCapLen() - 4));
}

QString DissectorEth::GetStrFCS(DissRes *packet){
    const uchar *fcs = packet->GetData() + packet->GetCapLen() -4;
    return QString::asprintf("0x%02x%02x%02x%02x",fcs[0],fcs[1],fcs[2],fcs[3]);
}

quint32 DissectorEth::GetCalculatedIntFCS(DissRes *packet){
    quint32 crc32_tab[] =
    {
    ...
    quint32 crc32val = 0;
    crc32val ^= 0xFFFFFFFF;
    for (qint32 i = 0; i < packet->GetCapLen(); i++)
        crc32val = crc32_tab[(crc32val ^ packet->GetData()[i]) & 0xFF] ^ ((crc32val >> 8) & 0x00FFFFFF);
    return ntohl(crc32val ^ 0xFFFFFFFF);
}


QString DissectorEth::GetCalculatedStrFCS(DissRes *packet){
    quint32 crc = this->GetCalculatedIntFCS(packet);
    uchar *p = (uchar*)&crc;
    return QString::asprintf("0x%02x%02x%02x%02x",p[0],p[1],p[2],p[3]);
}
...
QString DissectorEth::MsgDst(DissRes *dissRes){
    DissResEth *dissResEth = ((DissResEth*)dissRes);
    return QString("Destination: ") + dissResEth->GetStrMacDst();
}

QString DissectorEth::MsgSrc(DissRes *dissRes){
    DissResEth *dissResEth = ((DissResEth*)dissRes);
    return QString("Source: ") + dissResEth->GetStrMacSrc();
}

QString DissectorEth::MsgType(eth_hdr *eth){
    QString strtype = "unknow";
    quint16 inttype = ntohs(eth->eth_type);
    switch (inttype) {
    case 0x0800:
        strtype = "IPv4";
        break;
...
QString DissectorEth::MsgLG(uchar *mac){
    uchar LGbit = (mac[0] & 0x02) >> 1;
    return QString::asprintf(".... ..%d. .... .... .... .... = LG bit: ",LGbit)
                + ((LGbit == 1)?"Locally administered address (this is not factory default)"
                              :"Globally unique address (factory default)");
}

QString DissectorEth::MsgIG(uchar *mac){
    uchar IGbit = (mac[0] & 0x01) >> 0;
    return QString::asprintf(".... ...%d .... .... .... .... = IG bit: ",IGbit)
                + ((IGbit == 1)?"Group address (multicast/broadcast)"
                              :"Individual address (unicast)");
}

void DissectorEth::DealFCS(ProTree *tree, DissRes *dissRes){
    DissResEth *dissResEth = ((DissResEth*)dissRes);
    QString str;
    if(DissectorEth::FlagGetValidateFCSBit() == 1 && !dissResEth->GetEthCRCRes()){
            str = QString("Frame check sequence : ") + DissectorEth::GetStrFCS(dissRes) + " incorrect , should be "
                    + DissectorEth::GetCalculatedStrFCS(dissRes);
            tree->AddItem("ethertype",str,dissRes->GetCapLen() - 4,dissRes->GetCapLen() -1);
            str = QString("FCS status : Bad");
            tree->AddItem("ethertype",str,dissRes->GetCapLen() - 4,dissRes->GetCapLen() -1,ProTree::NEW);
            tree->Pop();
    }
}
...
//Flag方法
void DissectorEth::FlagSetValidateFCSBit(uchar option){
    if(option == 0){
        DissectorEth::flags &= 0xfffffffe;
    }else{
        DissectorEth::flags |= 0x00000001;
    }
}

uchar DissectorEth::FlagGetValidateFCSBit(){
    return (DissectorEth::flags & 0x00000001) >> 0;
}
